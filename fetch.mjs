...
// Ключи категорий у них и у нас совпадают почти везде, но не всюду: прилетело
// "tools", а у нас категория называется "tool", и второй бонус дня молча потерялся.
// Сверено по всему их архиву — 1716 записей, 38 различных категорий. Расходятся ровно
// три: tools/tool, food/meal и gatherergear/gatherer; остальные 35 (всё оружие, вся
// броня, сумки, плащи, зелья, пять линий переработки) сходятся точно.
// Порядок проб: как есть → псевдоним → без -s → с -s. Что не легло — возвращаем
// отдельным списком, чтобы сайт честно сказал «подставлен частично», а не делал вид,
// что всё на месте.
const SAGE_ALIAS = { tools:'tool', food:'meal', foods:'meal', gatherergear:'gatherer',
  mounts:'mount', potions:'potion', meals:'meal', capes:'cape', bags:'bag' };
...
async function main(){
  ...
  const matIds = [
    ...
    ...(items.farmIds||[]),    // семена, детёныши и взрослые животные: нужны разделу «Острова»
  ];
